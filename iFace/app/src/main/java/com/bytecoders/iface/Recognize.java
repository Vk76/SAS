...
public class Recognize extends AppCompatActivity implements CvCameraPreview.CvCameraViewListener{
...
//        nomes[1]=mAuth.getCurrentUser().getEmail().split("@+")[0];
        ...
        new AsyncTask<Void,Void,Void>() {
            ...
            protected Void doInBackground(Void... voids) {
                try {
                    faceDetector = TrainHelper.loadClassifierCascade(Recognize.this, R.raw.frontalface);
...
//                        faceRecognizer.load(f.getAbsolutePath());
    ...
    private void recognize(opencv_core.Rect dadosFace, Mat grayMat, Mat rgbaMat) {
        ...
        if (prediction == -1 || acceptanceLevel >= ACCEPT_LEVEL) {
            ...
            if(count>=3)
            {
                Intent intent = new Intent(getBaseContext(),WiFinder.class);
                startActivity(intent);
            }
            name = nomes[prediction] + " - " + (100-acceptanceLevel);
